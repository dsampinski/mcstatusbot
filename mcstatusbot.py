...
async def com_ping(ctx:discord.Interaction):
...
async def com_add(ctx:discord.Interaction, address:str, name:str|None=None):
    logging.info(f'{ctx.user} ran $add {address} {name} in {ctx.guild or "DM"} ({ctx.guild.id if ctx.guild is not None else ""})')
    if ctx.guild is None:
        logging.info(f'{ctx.user} is in a DM channel')
        return
    if str(ctx.user.id) != config["adminId"] and not ctx.user.guild_permissions.manage_channels:
        logging.info(f'{ctx.user} does not have permission to manage channels')
        try: await ctx.response.send_message('You do not have permission to manage channels', ephemeral=True)
        except Exception: pass
        return
    if str(ctx.user.id) != config["adminId"] and '.' not in address[1:len(address)-2]:
        logging.info(f'{ctx.user} entered an invalid address')
        try: await ctx.response.send_message('You have entered an invalid address', ephemeral=True)
        except Exception: pass
        return
    ...
    if db.getGuildServers(ctx.guild.id, address):
        ...
        try: await ctx.response.send_message('Server is already added', ephemeral=True)
        except Exception: pass
        lock.release(ctx.guild.id)
        return
    if str(ctx.user.id) != config["adminId"] and db.countServers(ctx.guild.id) >= config["serversPerGuild"]:
        logging.info(f'{ctx.guild} ({ctx.guild.id}) reached maximum amount of servers')
        try: await ctx.response.send_message('Reached maximum amount of servers in this guild', ephemeral=True)
        except Exception: pass
        lock.release(ctx.guild.id)
        return
    
    try:
        _ = await JavaServer.async_lookup(address)
        ...
        try: await ctx.response.send_message('Error: ' + str(e), ephemeral=True)
        except Exception: pass
    else:
        ...
        try: await ctx.response.send_message(f'Added {address}\'s status to this guild', ephemeral=True)
        except Exception: pass
    lock.release(ctx.guild.id)
# @com_add.error
...
@bot.tree.command(name='rem', description='Removes a server\'s status')
async def com_rem(ctx:discord.Interaction, address:str):
    logging.info(f'{ctx.user} ran $rem {address} in {ctx.guild or "DM"} ({ctx.guild.id if ctx.guild is not None else ""})')
    if ctx.guild is None:
        logging.info(f'{ctx.user} is in a DM channel')
        return
    if str(ctx.user.id) != config["adminId"] and not ctx.user.guild_permissions.manage_channels:
...
@bot.tree.command(name='list', description='Lists added servers')
async def com_list(ctx:discord.Interaction):
    logging.info(f'{ctx.user} ran $list in {ctx.guild or "DM"} ({ctx.guild.id if ctx.guild is not None else ""})')
    if ctx.guild is None:
        logging.info(f'{ctx.user} is in a DM channel')
        return
    if str(ctx.user.id) != config["adminId"] and not ctx.user.guild_permissions.manage_channels:
    ...
    try: await ctx.response.send_message(addresses, ephemeral=True)
    except Exception: pass
    lock.release(ctx.guild.id)
...
async def update(guild:discord.Guild, server):
    ...
    try:
        lookup = await JavaServer.async_lookup(server["address"])
